...
removing the expected privacy benefits of coinjoin. A similar vulnerability was
also reported in late 2024 (see [Newsletter #333][news333 coinjoin]).

Also in January, Abubakar Sadiq Ismail [revisited][news336 reserve] a Bitcoin
Core bug discovered by Antoine Riard in 2021 that causes block templates to
reserve 2,000 vbytes for the coinbase instead of 1,000. See [Newsletter
#342][news342 vbytes] for the February fix.
...
That same week, Riard [disclosed][news339 cycling] an additional vulnerability
using the [replacement cycling][topic replacement cycling] attack. An attacker
could exploit it by [pinning][topic transaction pinning] a victim’s unconfirmed
transaction, receiving non-propagating replacements, and selectively mining the
victim’s highest-fee version. Optech determined that this scenario required rare
conditions and would be difficult to sustain without being detected.

In February, Morehouse [disclosed][news340 htlcbug] a second LDK vulnerability
where if many HTLC had the same amount and payment hash, it would fail to settle
...
routing revenue until the bug was fixed in 0.1.1 (see Newsletter [#340][news340
htlcfix]).

In March, Morehouse [announced][news344 lnd] the responsible disclosure of a
fixed LND vulnerability affecting versions before 0.18. An attacker with a
...
highlighted gaps in the Lightning specification, which was later corrected (see
...
91842 could leave nodes with different UTXO sets, depending on whether or not
...
denial-of-service issue where an attacker could repeatedly request historic
...
vulnerability affecting older versions of Eclair where an attacker could
...
fixed in 30.0, see Newsletters [#361][news361 four], [#363][news363 four],
...
exception in a specific full-capacity stack edge case. It was found via
...
- **Lowering the minimum relay feerate:** After lowering the minimum
  ...
  sats/vB. Bitcoin Core developers [proposed][news366 lower feerate] adjusting the
  default minimum relay feerate. The Bitcoin Core 29.1 release lowered the
  default minimum relay feerate to 0.1 s/vB in early September.
...
- [LDK v0.1][] added support for both sides of the LSPS channel open negotiation
  protocols, [BIP353][] Human Readable Names resolution, and reduced on-chain
  fee costs when resolving multiple [HTLCs][topic htlc] for a single channel
  force-closure.
...
- [Bitcoin Inquisition 29.1][] adds support for `OP_INTERNALKEY`, a type of
  [covenant][topic covenants].
...
- [Bitcoin Core 30.0][] made multiple data carrier (OP_RETURN) outputs standard,
  ...
  sat/vb, added an experimental IPC mining interface for [Stratum v2][topic
  pooled mining] integrations, and removed support for legacy wallets in favor
  of [descriptors][topic descriptors].
...
[news344 lnd]: /en/newsletters/2025/05/09/#bip30-consensus-failure-vulnerability
